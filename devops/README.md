...
DEPLOY_ENV=prod
DEPLOY_HOST=knowledge-curator.lundandco.net
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for knowledge-curator.lundandco.net, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    knowledge-curator.lundandco.net:
      ansible_user: root
      host: knowledge-curator
      hostname: knowledge-curator.lundandco.net
```
...
Deploy the stack defined in `devops/stacks/knowledge-curator.lundandco.net.yml` to the remote server with:
