...
class SM2Algorithm:
    ...
    def _calculate_interval(
        cls, quality: int, repetitions: int, ease_factor: float, previous_interval: int
    ) -> int:
        """
    ...
    def estimate_success_probability(
        cls, days_since_review: int, interval: int, ease_factor: float
    ) -> float:
        """
    ...
    def calculate_optimal_review_time(
        cls, interval: int, ease_factor: float, target_probability: float = 0.9
    ) -> int:
        """
    ...
    def adjust_for_time_constraint(
        cls, items: list, available_time: int, average_time_per_item: int = 60
    ) -> list:
        """
