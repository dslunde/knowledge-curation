...
class ReviewScheduler:
    ...
    def _apply_order_strategy(
        ...
        if strategy == cls.ORDER_URGENCY:
            ...
            return sorted(
                items,
                key=lambda x: x.get('sr_data', {}).get('ease_factor', 2.5)
            )
    ...
    def create_learning_session(
        ...
        session = {
            ...
            'metadata': {
                ...
                'new_items': sum(1 for i in session_items if i.get('sr_data', {}).get('repetitions', 0) == 0),
                'review_items': sum(1 for i in session_items if i.get('sr_data', {}).get('repetitions', 0) > 0),
                'difficulty_distribution': cls._calculate_difficulty_distribution(session_items)
            }
        }
    ...
    def optimize_review_time(
        ...
        recommendations = {
            'best_review_times': [],
            'avoid_times': [],
            'optimal_session_length': cls._calculate_optimal_session_length(historical_performance),
            'suggested_schedule': []
        }
        ...
        recommendations['suggested_schedule'] = cls._create_weekly_schedule(
            best_hours[:3] if best_hours else [],
            user_preferences
        )
        ...
        for day in days:
            if day in available_days and best_hours:
                ...
                schedule.append({
                    ...
                    'duration': user_preferences.get('session_duration', cls.DEFAULT_SESSION_DURATION)
                })
    ...
    def get_adaptive_intervals(
        ...
        return {
            ...
            'adjustment_reason': cls._get_adjustment_reason(success_rate, adjustment_factor)
        }
