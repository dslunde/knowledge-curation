...
class PerformanceTracker:
    """Track and analyze spaced repetition performance metrics."""

    # Performance thresholds
    MASTERY_THRESHOLD = 21  # Days interval to consider item "mastered"
    STRUGGLING_THRESHOLD = 3  # Consecutive failures to identify struggling
    ...
    def _calculate_learning_velocity(cls, history: list[dict]) -> dict[str, float]:
        ...
        for item_id, reviews in items_data.items():
            if reviews:
                ...
                if len(reviews) >= 2:
                    ...
                    growth = latest_interval / first_interval if first_interval > 0 else 0
                    interval_growths.append(growth)

        # Calculate time span
        date_range = [
            datetime.fromisoformat(r['date']) for r in history
        ]
        if date_range:
        ...
        return {
            ...
            'mastery_rate': round(mastered_items / total_items * 100, 1) if total_items > 0 else 0,
            'average_interval_growth': round(statistics.mean(interval_growths), 1) if interval_growths else 0
        }
    ...
    def _analyze_difficulty_patterns(cls, history: list[dict]) -> dict[str, any]:
        ...
        return {
            'distribution': {
                k: len(v) for k, v in difficulty_groups.items()
            },
            'struggling_items': struggling_items,
            'average_ease_factor': round(
                statistics.mean([r.get('ease_factor', 2.5) for r in history]),
                2
            )
        }
