...
class ReviewQueueView(BrowserView):
    ...
    def get_queue_stats(self):
        ...
        stats = {
            ...
            'learning_items': sum(1 for i in items if 0 < i['sr_data']['repetitions'] < 3),
            'mature_items': sum(1 for i in items if i['sr_data']['repetitions'] >= 3),
            'critical_items': sum(1 for i in items if i['sr_data']['retention_score'] < 0.5)
        }
...
class ReviewCardView(BrowserView):
    ...
    def _get_content_for_review(self, obj):
        ...
        return {
            ...
            'full_content': getattr(obj, 'text', {}).output if hasattr(obj, 'text') else ''
        }
