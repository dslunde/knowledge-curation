...
def suggest_connections(state_change):
    ...
    try:
        ...
        logger.error(f"Failed to suggest connections: {str(e)}")
...
def record_start_time(state_change):
    ...
    try:
        ...
        logger.error(f"Failed to record start time: {str(e)}")
...
def record_completion_time(state_change):
    ...
    try:
        ...
        logger.error(f"Failed to record completion time: {str(e)}")
...
def handle_workflow_transition(obj, event):
    ...
    logger.info(
        f"Workflow transition: {workflow_id}/{transition_id} "
        f"for {obj.absolute_url()}"
    )
