...
from .relationships import RelationshipType, RelationshipManager, RelationshipMetadata
...
class GraphOperations:
    ...
    def create_relationship(self, source_uid: str, target_uid: str, 
        ...
        # Create edge
        edge = Edge(source_uid, target_uid, relationship_type.value, weight, **properties)
        ...
        if success:
            ...
            if RelationshipMetadata.is_bidirectional(relationship_type):
                reverse_edge = Edge(target_uid, source_uid, relationship_type.value, weight, **properties)
    ...
    def remove_relationship(self, source_uid: str, target_uid: str, 
        ...
        if success:
            self._log_operation('remove_relationship', {
                'source': source_uid,
                'target': target_uid,
    ...
    def merge_nodes(self, primary_uid: str, secondary_uid: str) -> bool:
        ...
        for edge in incoming_edges:
            if edge.source_uid != primary_uid:  # Avoid self-loops
                new_edge = Edge(
                    edge.source_uid, primary_uid, edge.relationship_type,
                    edge.weight, **edge.properties
                )
                self.graph.add_edge(new_edge)
    ...
    def clone_subgraph(self, root_uid: str, max_depth: int = 2, 
                      relationship_types: Optional[List[RelationshipType]] = None) -> Graph:
    ...
    def suggest_connections(self, uid: str, limit: int = 10) -> List[Tuple[str, RelationshipType, float]]:
        ...
        Returns:
            List of (target_uid, relationship_type, confidence) tuples
        """
        node = self.graph.get_node(uid)
        if not node:
            return []
        ...
        for edge in self.graph.get_edges_from_node(uid):
            existing_targets.add(edge.target_uid)
            
        # Find candidates based on shared connections
        connection_counts = {}
        ...
        for candidate_uid, shared_connections in connection_counts.items():
            ...
            if rel_suggestions:
                ...
                # Calculate overall confidence based on shared connections
                confidence = min(0.9, (shared_connections / 10.0) * rel_confidence)
                
                # Find matching RelationshipType enum
                rel_type = None
