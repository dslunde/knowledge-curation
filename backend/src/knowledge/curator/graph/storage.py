...
class GraphStorage:
    ...
    def save_graph(self, graph: Graph):
        ...
        for uid, node in graph.nodes.items():
            node_data = {
                ...
                'created': node.created.isoformat() if hasattr(node.created, 'isoformat') else node.created,
                'modified': node.modified.isoformat() if hasattr(node.modified, 'isoformat') else node.modified
            }
        ...
        for edge in graph.edges:
            edge_data = {
                ...
                'created': edge.created.isoformat() if hasattr(edge.created, 'isoformat') else edge.created
            }
        ...
        for brain in brains:
            ...
            if node:
                ...
                node = Node(
                    uid=uid,
                    title=brain.Title,
                    node_type=node_type_map.get(brain.portal_type, NodeType.RESEARCH_NOTE),
                    url=brain.getURL(),
    ...
    def query_nodes(self, node_type: Optional[str] = None, 
        ...
        if node_type:
            storage = self._get_storage()
            if 'by_type' in storage['indexes'] and node_type in storage['indexes']['by_type']:
    ...
    def query_relationships(self, relationship_type: Optional[str] = None,
                          ...
                          target_uid: Optional[str] = None) -> List[Edge]:
    ...
    def export_graph(self, format: str = 'json') -> str:
        ...
        if format == 'json':
            ...
            # Edges
            for edge in graph.edges:
                graphml.append(f'    <edge source="{edge.source_uid}" target="{edge.target_uid}">')
                graphml.append(f'      <data key="type">{edge.relationship_type}</data>')
